...
class Student_login_page(tk.Tk):
    ...
    def login_action(self):
        ...
        get_id=self.db.check_id_pass(self.id_entry.get(),self.pass_entry.get())
        if not get_id:
            messagebox.showerror("wrong","Invalid Id Or Password....")
            self.id_entry.configure(highlightcolor="red",highlightbackground="red")
            self.pass_entry.configure(highlightcolor="red",highlightbackground="red")
             
        else:
            messagebox.showinfo("hello","Welcome To Our System...")
    ...
    def __init__(self):
        ...
        self.login_img=Image.open("images//login.png").resize((100,100))
        ...
        self.header_text=tk.Label(self.header_login_frame,text="Login Page",bg=self.bg_color,fg="white",font=("Bold",18))
        
        self.header_text.place(x=0, y=0,width=400)
        
        self.header_image = tk.Label(
            self.header_login_frame,image=self.login_img,bg="#FAF9F6")

        self.header_image.place(x=142, y=50)
        
        self.id_entry_label = tk.Label(text="Enter Id Number", font=(
            "Impact", 15), fg=self.bg_color, bg="#FAF9F6")

        self.id_entry_label.place(x=180,y=190)
        
        self.id_entry=tk.Entry(self.header_login_frame,font=("Bold",15),justify=tk.CENTER,relief="solid",highlightbackground=self.bg_color,highlightthickness=3)
        
        
        self.id_entry.place(x=80,y=190)
        
        self.id_entry.bind(
            "<KeyRelease>", lambda e: self.remove_highlight_warning(self.id_entry))


        self.pass_entry_label = tk.Label(text="Enter Password", font=(
            "Impact", 15), fg=self.bg_color, bg="#FAF9F6")

        self.pass_entry_label.place(x=180, y=270)
        ...
        self.login_button = tk.Button(text="Login",command=self.login_action ,font=("arial", 18), width=10, relief="flat",
        ...
        self.forget_pass = tk.Button(text="Forget Password ⚠️", font=(
            "arial", 10), bg="#FAF9F6", fg="red",command=self.forgettin_pass, relief="flat", bd=0, activebackground="#FAF9F6",activeforeground="red")
